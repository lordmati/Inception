...
if ! wp user list --allow-root --path="/var/www/html" | grep -q "^\s*[0-9]\+\s\+$USER_TEST\s"; then
    echo "Creating WordPress user '$USER_TEST'..."
    wp user create "$USER_TEST" "$USER_EMAIL" \
        --role=author \
        --user_pass="$USER_PASS" \
        --allow-root \
        --path="/var/www/html"
    ...
    echo "User '$USER_TEST' already exists. Skipping creation."
fi
